...
- Upstream's README asks each PR to bump the version in `package.json`,
  `src-tauri/Cargo.toml` and `src-tauri/tauri.conf.json` together. The fork no longer
  carries a version bump of its own (retired 2026-08-20 — see PATCHES.md), so
  `integration` just takes upstream's number. If a maintainer wants a bump on a PR, add
  it on that branch with the `version-bump` skill.
...
`rerere` is enabled, so conflicts resolved once replay automatically on later rebases.

### Check that the rebuild kept the content

A rebase can leave a commit's *message* intact while its diff shrinks to nothing —
already-applied hunks get dropped silently, and nothing warns you. That is how
`fix(pnpm)` reached 2026-08-31 still describing a `pnpm-workspace.yaml` that had not
existed on `integration` since the 2026-08-20 rebuild. Diff the rebuilt stack against the
pre-rebuild state and read the file list, not just the commit subjects:

```bash
git diff sync/<previous-date> integration --stat
```

Everything upstream changed shows up, so scan for the reverse: files a carried or
upstream-bound commit is *supposed* to touch that are missing. A quicker per-commit
check is `git show <sha> --stat` — a commit whose message promises three files and whose
stat shows one has been gutted.

If `rerere` replays a resolution recorded in a different context, check the result before
continuing: a resolution recorded while rebasing `integration` has the whole stack
applied, and replaying it onto a standalone PR branch can drag in text from a commit that
branch does not carry. And if you resolve a conflict wrongly and then amend, `rerere` has
already stored the bad resolution — fix the postimage in `.git/rr-cache/<hash>/` or the
next rebase silently replays the bug.
