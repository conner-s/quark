...
![version](https://img.shields.io/badge/version-0.13.1-00ff41) ![license](https://img.shields.io/badge/license-AGPL--3.0-blue) ![stack](https://img.shields.io/badge/Tauri-v2-ffc131)
...
![Quark screenshot](docs/screenshots/quark.png)
...
Multi-platform bundles are produced by the GitLab CI pipeline ([.gitlab-ci.yml](.gitlab-ci.yml)) on tagged releases: Linux (`.deb` / `.AppImage` / `.rpm`), Flatpak, Windows (`.msi` / `.exe`), and macOS (`.dmg`). A Flatpak manifest lives under [flatpak/](flatpak/) (app ID `zone.derg.quark`).
...
- Run `pnpm test` and `cargo test` (from `src-tauri/`) before opening a merge request.
