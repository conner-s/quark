...
- [x] Text selection; o on a message moves the cursor into the message for selection of the text — `o` in the timeline calls `enter-text-select`, which makes the selected message body `contenteditable` (with all inserting keys trapped) so the browser maintains a real caret. The native I-beam is hidden and the keymap layer keeps a 1-character selection at the cursor so the highlight reads as a vim-style block cursor (the `--text-select-block` class sets `caret-color: transparent`; `primeBlockSelection` extends a collapsed selection by one character forward, falling back to backward at end-of-content). Normal-mode movement does collapse→move→re-prime so the block slides one character per keystroke; Visual extends the selection from a fixed anchor, then Visual→Normal collapses back to the focus end. Block-cursor coloring is theme-driven: Normal renders the 1-char block with `--cursor` background and `--bg` foreground (inverse-video like vim's "Cursor" group); the `--text-select-visual` modifier toggled on Visual overrides `::selection` back to `--selection-bg`/`--selection-fg` so the user can tell "cursor" apart from "selection". `textSelectMode` ("message" | "compose" | null) on `AppState` tracks the submode; `app/text_select.ts` owns enter/exit and selection helpers.
  - [x] If in text selection mode and visual mode, 'y' should copy selected text and '>' should insert selected text into the text box with md quote prefix i.e. `> quoted text here` — `handleTextSelectKeydown` resolves keys through `keymapManager`, then switches on the action name (`nav-up`/`nav-down`/`nav-left`/`nav-right`, `copy-message`, `quote-selection`, etc.) — so any user remap (e.g. ijkl-nav) applies inside text-select too. `v` enters Visual, where movement actions extend the selection via `Selection.modify("extend", …)` on the message body or by tracking anchor/focus on the input. `y` copies the current selection and exits; `>` prefixes each line with `> ` and appends to the compose box, then drops to Insert. Anything not explicitly handled is `preventDefault`'d so Backspace/Delete/Enter/Tab can't mutate the focused region.
  - [x] If I'm in insert mode and the compose box is not empty, I should enter text select mode in the compose box — the `modeManager` listener detects Insert→Normal with `input.getValue().length > 0` and calls `enterComposeTextSelect()` instead of blurring; the field stays focused and key routing flips to the text-select handler.
  ...
  - [x] In text select mode in normal mode in the compose box, 'p' should paste into the compose box at the cursor position — `p` exits text-select with the caret position preserved and drops into Insert; the user finishes with Ctrl/Cmd+V, which the browser pastes at the cursor natively. We don't read the clipboard programmatically (macOS pops a permission dialog every time, same reason as the existing `paste-to-input` action outside text-select).
...
- [x] Forward-paging messages continues scrolling infinitely, seemingly due to anchoring issue — root cause was a stale `_scrolledUp` flag: the capture-phase image-load handler reads `_scrolledUp` synchronously when cached images in freshly rendered DOM resolve, but `_scrolledUp` only updates inside the async scroll-event handler. After `_rebuildWindow`/`appendMessages`/`prependMessages` the stale `false` value made the load handler snap the viewport to the bottom, which re-triggered `_handleScrollNearBottom` in a loop. Fix: synchronously recompute `_scrolledUp` after each scrollTop/scrollHeight mutation (same pattern already used in `scrollToMessage`)
- [x] Highlight + ctrl/cmd+c doesn't copy — Normal/Visual keymap was matching the bare `c`/`x`/`a`/`v` key and calling preventDefault on Ctrl/Meta combos. Global keydown now bails out (and clears any pending multi-key sequence) when Ctrl or Meta is held, letting browser-native copy/cut/paste/select-all through. Ctrl+K and Ctrl+[ remain intercepted above this guard.
- [x] First load of react emoji picker does not show all custom emojis — fixed a race in `openQuickReactPicker` where each per-thumbnail `setCustomEmoji` call rebuilt from a freshly-mapped array that dropped earlier resolutions; placeholders are now mutated in place so every concurrent resolution sees the full set
- [x] React picker does not show separate category for custom emotes — `QuickReactPicker` now has a dedicated "custom" category button (🧩) that appears only when `setCustomEmoji` supplies entries; built-in category filters hide custom emoji so they only appear under the custom category
- [x] Some system emojis do not render correctly, possibly a font issue? — `--font-family` and `--font-family-messages` now include emoji fallbacks (Apple/Segoe/Noto/Twemoji + CSS `emoji` generic) so glyphs missing from the mono fonts render in color instead of tofu boxes
- [x] Some highlight colors don't seem to change with theme. — `--focus-color` now derives from `--accent-primary` by default, so the focused-message left bar and other focus rings track the active theme (catppuccin mocha now uses mauve instead of hardcoded green)
- [x] File downloads should save to a chosen location rather than opening directly — the `quark:open-file` handler now opens the OS save-as dialog via `@tauri-apps/plugin-dialog`, then calls a new `save_media_to_path` Rust command that downloads from the homeserver and writes the bytes to the user-selected path; browser/mock dev mode falls back to a synthetic `<a download>` so the UX is testable without Tauri. Video inline playback and the image-lightbox download are unaffected
