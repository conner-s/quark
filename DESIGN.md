...
**room-open performance fix.** The initial iteration also routed the *live timeline* through the event cache. That regressed room-open badly: matrix-sdk 0.9's `RoomEventCache` deserializes the room's **entire** persisted linked-chunk into memory on first access (no lazy/partial load), and deep searches persist tens of thousands of events into that chunk — so first-open of a heavily-searched room took **40+ seconds** (measured: 50,128 cached events → 48s, dominated by `room.event_cache().await` itself). Fix: `open_room_timeline`/`load_older_timeline` now use raw `room.messages()` (transient, bounded, does **not** persist), with a per-room `prev_batch` token tracked server-side in `TimelineTokens`. The event cache is touched only by search now, so room-open cost is independent of cache size. `pnpm test` green (468); `cargo test` green.

**instant re-open (frontend cache).** With the live timeline on `room.messages()`, *every* open (re-opens included) paid a homeserver round-trip — profiled at a steady **~2.2–2.6s per open**, consistent per-server, so it's network/server latency, not local work (`aggregate` was ~9ms). Fix is a per-room frontend cache (`_roomTimelineCache`, capped at 200 events): `selectRoom` paints the cached tail **synchronously** on revisit (skips the skeleton) and runs the authoritative `openRoomTimeline` in the background, re-rendering only if the head changed (no flicker, scroll preserved). The sync stream appends new messages to every already-cached room's tail so revisits aren't stale. Initial fetch trimmed 100 → 50 events to shave the unavoidable first-ever open. **Images:** message media now has an in-memory blob-URL cache (`_messageMediaCache`, mirroring avatars) — previously every render re-fetched + re-decoded media over IPC (~1s even on a disk-cache hit); now `timelineEventToMessage` pre-fills the cached blob URL so revisited images paint with the text, and downloads kick off at paint time instead of after the background fetch. Only the *first* open of a room in a session still hits the network (~1.5–2s, homeserver-bound); a prefetch-on-room-list-load step could remove that too if wanted.

**cache footprint controls.** Audited the real footprint: the **event-cache SQLite** (search persistence) was the dominant store on disk (measured ~466 MB after deep searches), while the speed caches added in 0.13.3 (`_messageMediaCache`, `_roomTimelineCache`) were unbounded in memory. Added a new `[cache]` config section (`image_memory_mb` default 150, `timeline_rooms` default 30) and surfaced everything in Settings → Media → Cache:
- **`_messageMediaCache` is now an LRU** (`MediaBlobCache`) bounded by a byte budget; evicted blob URLs are `URL.revokeObjectURL`'d to actually free memory. (Edge: an evicted image still off-screen in the *current* room shows broken until re-open; the default budget makes this rare and cross-room opens self-heal via re-download.)
- **`_roomTimelineCache` is LRU-capped** by room count (insertion order = recency; every open re-`set`s as newest).
- **Settings → Cache** now shows media-cache size, **search-cache (event-cache SQLite) size**, and the two memory caps; with **[clear media cache]** and **[clear search cache]** buttons. Clearing search cache calls `clear_event_cache` (`EventCacheStore::clear_all_rooms_chunks` at the store level — no per-room reload); note SQLite frees pages for reuse but the file shrinks fully only after a restart compaction. Saving the caps applies them live via `applyCacheConfig` (no restart).
...
- [x] **Late-key re-decryption** — the event cache stores events with their decrypt-state at sync time and (in 0.9) never retries when keys arrive later, so the initial page could stay `🔒 unable to decrypt` after verification. Two-part fix: **(A)** the live timeline loads via raw `room.messages()`, which decrypts fresh on every fetch — so a reload reflects current keys (no per-event re-decrypt pass needed); **(B)** a `room_keys_received_stream` listener emits `quark://sync/room_keys`, and the frontend reloads the displayed room (`reloadCurrentRoomTimeline`, preserves scroll, skipped in context view) so it refreshes automatically without a manual re-open. *(0.13.0: the same listener now also triggers a debounced `refreshRooms()` so the room **list** — names / last-message previews / avatars that were UTD — repaints across all rooms when keys arrive, e.g. the flush right after verifying a new session, instead of needing a relaunch.)*
...
## Verification queue — 0.11.3 scroll-stability batch

Branch `fix/scroll-stability`. Reworks how the timeline holds scroll position so room loads and window refocus stop jittering. `pnpm test` green (460).

### Cross-platform — needs manual verification
- [x] **Stick-to-bottom via ResizeObserver** — while the user is at the live tail, late content growth (images decoding, custom emoji/stickers resolving) re-pins to the bottom in one coalesced write per frame, replacing the per-image `load`→scrollToBottom handler and the 150 ms post-render timer chain in `setMessages`. *Verify: open a room with several images — it should settle at the bottom without the staircase jitter as each image decodes. Scrolling up mid-load must not get yanked back down.*
- [x] **Stick keyed on a dedicated intent flag, not `_scrolledUp`** — the observer follows `_stickToBottom`, which disengages on any upward movement and re-engages only at the very bottom. Keying it on `_scrolledUp` (40px deadzone) let a content resize re-pin the user while they nudged up within that band, making it impossible to scroll up while media was still loading. `_scrolledUp` keeps its 40px hysteresis for the jump-to-latest button. *Verify: while images are still loading on a fresh room, scrolling up holds and history pagination at the top works.*
- [x] **Refocus no longer scrolls** — resuming GIFs on window focus reassigns `img.src`, which used to re-fire the capture-phase `load` handler once per GIF and snap to the bottom. With that handler gone, refocusing the window holds position. *Verify: scroll up in a room with GIFs, switch to another app and back — the timeline should stay put.*
- [x] **Random upward jump on load** — `updateMessageMedia` now compensates scrollTop whenever a resolving image's top is above the viewport top, not only when it is *entirely* above the fold, so an image straddling the top edge no longer shoves the read position to an earlier point. *Verify: load a room while scrolled into history with images above the fold still downloading; position should hold.*
- [x] **GIF flicker / scrolled-up landing** — GIFs frequently arrive with no usable dimensions (the Klipy provider falls back to `0×0`, and remote clients often omit `info.w/h`), so `0` is falsy at render and no layout space is reserved; the Blob URL then decodes async and grows the box from zero, and with several GIFs that reads as a staircase of jumps. Three fixes: (A) the stick-intent disengage now checks "at the bottom" *first*, so a content **shrink** that clamps scrollTop downward (a GIF decoding shorter than a reserved box) is no longer mistaken for a user scroll-up — that was leaving the user stranded above the tail; (B) the Rust `send_gif` now probes real pixel dimensions from the GIF header bytes it already downloads (`gif_dimensions`), so sent GIFs always carry accurate `w/h`; (C) `updateMessageMedia` locks the box from the decoded natural size for any media that arrived unsized and backfills the buffer, so received GIFs missing `info` settle in one reflow and reserve correctly on later re-renders. *Verify: a room with several GIFs in the initial window opens at the bottom without flicker or a scrolled-up landing.*
- [x] **Prefetch history a viewport early** — the near-top trigger fired at the very edge (`scrollTop < 80`), so the async server fetch only started once the user had run out of content — the visible pause-then-settle. The trigger distance is now one viewport (`prefetchMargin = clientHeight`) **but only for the server-fetch branch** (`_renderStart === 0`): that fetch is async, so overlapping it with the scrolling still to come hides its latency. The buffered-window extend (`_renderStart > 0`) stays edge-triggered (80px) — it's a synchronous `innerHTML` teardown plus a `scrollTop` write, and firing it a full viewport early lands inconsistently mid-fling (peak velocity, `overflow-anchor: none` not self-correcting), which read as an intermittent "huge jump" on load. The momentum kill remains the safety net for flings that outrun the network. Guarded on `!_stickToBottom` so a room opened at the tail — or one shorter than the viewport, where `scrollTop` is 0 while pinned — doesn't fire a spurious history fetch before the user has scrolled up at all. *Verify: scroll up at a normal pace in a long room — older messages should appear with no pause at the top; opening a room or a short room should not trigger a history load on its own.*
- [x] **Duplicate back-to-back history fetches** — after enabling the viewport-early prefetch, a media-heavy page could fire a *second* server fetch immediately after the first finished (sequential, not concurrent — the `paginationLoading` guard held). Root cause: prepended events whose senders omitted `info.w/h` come back with null `media_width/height`, so their `<img>` rendered at ~0 height; the prepend's `scrollTop = oldScrollTop + scrollHeightDelta` restore then added almost nothing (observed: scrollTop went 644→556 across a 48-message prepend, not 644+thousands), leaving the user still inside the one-viewport prefetch band — and `prependMessages` unconditionally re-armed `_scrollTopFired`, so the next scroll tick re-fired. The old 80px edge trigger masked it (556 ≫ 80 sat outside the zone). Two fixes: (A) **reserve placeholder height** for media with no event dimensions — a `--unsized` class (`min-height: 200px` for images, `--sticker-max-size` for stickers) so a prepend actually grows `scrollHeight` and advances `scrollTop`; `updateMessageMedia` drops the class and locks the real size on decode (also removes the residual pagination jump). (B) **conditional re-arm**: `prependMessages` only clears `_scrollTopFired` if the restore landed *outside* the band (`scrollTop >= clientHeight`); otherwise it stays latched so a barely-moved viewport can't immediately re-fire. *Verify: scroll slowly up through a media-heavy room — exactly one fetch per approach (watch the `[pagination] #n` console logs), no stacked loads, and dimensionless images reserve space instead of popping in.*
- [x] **Kill momentum on history fetch** — a high-momentum fling into the top would chain-trigger repeated server loads: WebKit keeps integrating inertial velocity across the async fetch and each prepend's scrollTop restore, so the fling re-hits the top and loads again, fighting the position fix. `_handleScrollNearTop` now kills the fling (one-frame `overflow-y: hidden`, gutter stays reserved so no flash; programmatic scrollTop still works) when a real fetch starts — but *not* on intermediate buffered-window extensions, which are synchronous and compensated, so flinging through history stays smooth. *Verify: fling hard into the top of a long room — it should stop at the top and load once, not stutter through several loads.*
- [x] **Pagination jump (top message slides to middle)** — when older messages page in at the top, their media is downloaded *after* the prepend, above the viewport. Because Blob URLs decode asynchronously, the old synchronous height measurement in `updateMessageMedia` read ~0 and missed the real growth, so the decoding media shoved the read position down. `updateMessageMedia` now compensates the running height delta on the actual `load` (not just synchronously) whenever the element's top is above the viewport top, keeping the read position fixed. General to images and GIFs alike. *Verify: scroll up to trigger a history load in a room with media above the fold — the message you were reading should stay put.*
- [x] **Rooms always open at the live tail** — dropped the auto-jump to the unread separator on room load. It centered the separator in a rAF before media decoded, and images growing in the band above it shoved it downward by a variable amount → a "random" mid-list landing. The unread marker is still drawn in-place; the user scrolls up or uses jump-to-latest to reach it. (Removed the now-unused `scrollToUnreadSeparator`.) *Verify: open a room with unread messages — it lands at the newest message, with the "new messages" divider visible in-place above.*

Deferred (documented, not done): enabling CSS `overflow-anchor` as a detected fast-path — scroll anchoring is not in any stable WKWebView/WebKitGTK release yet (merged to WebKit trunk Feb 2026, absent from Safari 26.5), and turning it on would re-introduce the double-correction flicker that `overflow-anchor: none` was set to avoid (commit bfceed6). Revisit once it ships in the bundled WebViews. Element-anchored prepend/cull restoration (vs. the current height-delta) also deferred — the synchronous delta is accurate in practice and the change is higher-risk.
...
- [x] **Flatpak Secret Service access** — the Flatpak manifest (`flatpak/zone.derg.quark.yml`) grants `--talk-name=org.freedesktop.secrets`. Without it the sandbox bus policy blocks the keyring, so `is_available()` returned false and every Flatpak login failed with "Secure storage is unavailable."
- [x] **No store wipe on a locked keyring** — `restore_session` returns a `RestoreOutcome` (`Restored`/`NoSession`/`Unavailable`/`Invalid`) instead of erroring uniformly. Only `Invalid` (missing key, dead token) triggers a frontend wipe; a locked/unreachable keyring (`Unavailable`) leaves all local state intact and shows the login screen with unlock guidance, so a transient lock at startup no longer destroys the encrypted crypto store (which would force device re-verification + a full re-sync).
- [x] **Android backup disabled** — `android:allowBackup="false"` in the Android manifest keeps the plaintext-fallback key/session files out of Auto Backup / device-transfer, confining them to the OS sandbox.
- [x] **Per-install keyring namespacing** — desktop keyring entries are tagged with a hash of the resolved data dir (`secrets.rs`), so two installs sharing one OS keyring (e.g. Flatpak + native) no longer collide on a single shared session/store-key while keeping separate on-disk stores. The over-long-value keyring error (`TooLong`, e.g. Windows Credential Manager's ~2560-byte cap) now maps to actionable guidance instead of an opaque message.

E2EE onboarding (done in 0.13.0):
- [x] **Verify-this-session prompt** — on startup (fresh login and session restore), if the current session isn't cross-signed and there's another device to compare against, a `VerificationPromptDialog` offers **Verify now / Not now / Never ask**. "Verify now" runs the existing SAS emoji-compare flow (`startVerification`); "Never" persists `general.prompt_session_verification = false` (re-enable in Settings → General). Gated in `maybePromptSessionVerification` (`app/actions/session.ts`), fired ~2.5s after sign-in so the device list / cross-signing status has settled.
...
- [ ] **iOS Keychain persists across uninstall** (Low, iOS-only) — unlike the app container, iOS Keychain items survive an app delete, so the store key + session linger after uninstall and a reinstall could silently auto-login with a stale token against a fresh (empty) store. The `keyring` crate does **not** set `kSecAttrSynchronizable`, so these never sync to iCloud Keychain / other devices — the persistence is local-only. A proper fix (mark the items `…ThisDeviceOnly` and/or clear them on first launch) needs platform-specific Keychain code that `keyring` v3's generic `Entry` API doesn't expose; deferred until iOS is an actual shipping target. (Desktop is unaffected; entries are now namespaced per data dir — see below.)
